...
func (m *Master) getTask(taskSeq int) Task {
	...
	DPrintf("m:%+v, taskseq:%d, lenfiles:%d, lents:%d",
		m, taskSeq, len(m.files), len(m.taskStats))
...
func (m *Master) schedule() {
	...
	for index, t := range m.taskStats {
		...
		case TaskStatusErr:
			...
			m.taskCh <- m.getTask(index)
		default:
			panic("t.status master schedule err")
		}
	}
	if allFinish {
...
func (m *Master) GetOneTask(args *TaskArgs, reply *TaskReply) error {
	...
	DPrintf("in GetOneTask, args:%+v, reply:%+v", args, reply)
	return nil
}
